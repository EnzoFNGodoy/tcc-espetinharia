...
{
    public partial class frmVerProdutosSaidaEstoque : Form
    {
        ItensSaidaEstoqueDAO itsSaiEstDAO = new ItensSaidaEstoqueDAO();
        DataGridViewImageColumn imgCol = new DataGridViewImageColumn();
        int idSaida;

        public frmVerProdutosSaidaEstoque(string id)
        {
            InitializeComponent();
            idSaida = Convert.ToInt32(id);
        }
        ...
        private void frmVerProdutosSaidaEstoque_Load(object sender, EventArgs e)
        {
            gvExibirProdutos.DataSource = itsSaiEstDAO.listarProdutosSaidaEstoqueDataTable(idSaida);
            atualizarGV();
            atualizarColunaFoto();
            txtIdSaidaEstoque.Text = idSaida.ToString();
        }
